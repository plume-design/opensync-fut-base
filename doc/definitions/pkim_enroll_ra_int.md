...
future timestamp as a string. This test tests the simple integer case.
...
- ensure that the device connects to EST server and issues a `simpleenroll`
  request
- the server responds with 503 and provides a `Retry-After` header with a simple
  integer value
