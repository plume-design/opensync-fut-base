# Testcase fsm_configure_test_dns_plugin
...
FSM and its DNS plugin should block traffic according to the policy set in the
DNS plugin. FSM monitors user DNS requests, and decides if the consequent DNS
...
DNS request is triggered on the client, but is blocked/dropped by the tested
FSM and DNS plugin according to the rules.
