...
Basic PKI certificate enroll/re-enroll flow test, but EST server provides
certificates that are about to expire. In this case, the `PKI_Config:status`
column should be set to `overdue` by the end of the test.
...
- ensure that the device connects to EST server and issues a `simpleenroll`
  request
- ensure that the certificate status in `PKI_Config` is set to `overdue`
...
`PKI_Config:status` is set to `overdue` and the certificate serial number
