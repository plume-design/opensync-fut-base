# Upgrade Manager - UM test suite

The `Upgrade Manager - UM` is responsible for correct and reliable firmware upgrades.

After receiving a trigger from the cloud, `UM` makes sure the device firmware upgrade process completes.

In case of FUT test cases, the cloud triggers are simulated by controlling the fields in the OVSDB table `AWLAN_Node`
which is the table that enables communication from the cloud to the device and vice-versa.
