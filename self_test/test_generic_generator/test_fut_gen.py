...
class TestFutTestConfigGenClass:
    ...
    def test_get_model_config_dir(self, devices):
        ...
        expected = f"{fut_base_dir}/config/test_case/model/{devices}"
    ...
    def test_load_module(self):
        testcase_module = "config.test_case.generic"
        expected = {}
        test_config_files = [
            "FSM_inputs",
            "UM_inputs",
        ...
        actual = FutTestConfigGenClass("PP403Z", "PP203X")._load_module(
            test_config_files,
            testcase_module,
        )
        assert expected == actual
    ...
    def test_load_module_no_module_fallback(self):
        testcase_module = "config.test_case.generic"
    ...
    def test_load_module_with_modules_list(self):
        testcase_module = "config.test_case.generic"
        expected = {}
        test_config_files = [
            "FSM_inputs",
        ...
        for cfg_file in test_config_files:
            ...
            spec.loader.exec_module(module)
            expected = {**expected, **module.test_inputs}
        actual = FutTestConfigGenClass("PP403Z", "PP203X", modules=["FSM", "UM"])._load_module(
            test_config_files,
            testcase_module,
        )
        assert expected == actual
    ...
    def test_load_configurations_generic_cfg(self):
        fut_base_dir = f"{Path(__file__).absolute().parents[2].as_posix()}"
        testcase_generic_dir = "config/test_case/generic"
        ...
        test_generic_cfg = FutTestConfigGenClass("PP403Z", "PP203X")._load_module(
            config_generic_files,
            testcase_generic_mod,
        )
        expected = Config(test_generic_cfg)
        actual, _, _ = FutTestConfigGenClass("PP403Z", "PP203X")._load_configurations()
        assert expected.cfg == actual.cfg

    @allure.title("Validate _load_configuration() method")
    def test_load_configurations_model_cfg(self, devices):
        fut_base_dir = f"{Path(__file__).absolute().parents[2].as_posix()}"
        testcase_model_dir = f"config/test_case/model/{devices}"
    ...
    def test_load_configurations_vendor_cfg(self, devices):
        ...
        test_vendor_cfg = {}
        dut_vendor_chipset = (
            FutTestConfigGenClass(devices, "PP203X")._load_model_capabilities(devices).get("wifi_vendor")
        )
        testcase_vendor_dir = f"config/test_case/platform/{dut_vendor_chipset}"
        ...
        test_vendor_cfg = FutTestConfigGenClass(devices, "PP203X")._load_module(
            config_vendor_files,
            testcase_vendor_mod,
        )
    ...
    def test_get_test_configs(self, devices):
        expected = FutTestConfigGenClass(devices, "PP203X").combined_configs
        actual = FutTestConfigGenClass(devices, "PP203X").get_test_configs()
        assert expected == actual
