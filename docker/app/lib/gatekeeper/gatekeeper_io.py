...
class Response:
    def __init__(self, client_req, logger=logger):
        ...
        with open("/var/www/app/lib/gatekeeper/gatekeeper_content.json", "r") as f:
            content = f.read()
            self.db = json.loads(content)

        with open("/var/www/app/lib/gatekeeper/gatekeeper_category.json", "r") as f:
            content = f.read()
    ...
    def formatted_attribute(self):
        if self.rtype == ReqType.IPv4:
            ipstr = socket.inet_ntop(
                socket.AF_INET, self.attribute.to_bytes(4, byteorder="big"),
            )
            return ipstr
    ...
    def fill_header(self):
        if self.rtype == ReqType.FQDN:
            ...
            self.fill_fqdn_reply_body(
                request_header, reply_redirect, reply_header.action,
            )
    ...
    def serialize(self):
        ...
        the_data = json_format.MessageToDict(
            self.response, preserving_proto_field_name=True,
        )
        present_data(the_data)
