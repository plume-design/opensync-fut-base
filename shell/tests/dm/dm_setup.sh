...
device_init &&
    ...
    raise "FAIL: device_init - Could not initialize device" -l "dm/dm_setup.sh" -ds

start_openswitch &&
    log -deb "dm/dm_setup.sh - OpenvSwitch started - Success" ||
    raise "FAIL: start_openswitch - Could not start OpenvSwitch" -l "dm/dm_setup.sh" -ds

restart_managers
    log -deb "dm/dm_setup.sh: Executed restart_managers, exit code: $?"

empty_ovsdb_table AW_Debug &&
    log -deb "dm/dm_setup.sh - AW_Debug table emptied - Success" ||
    raise "FAIL: empty_ovsdb_table AW_Debug - Could not empty AW_Debug table" -l "dm/dm_setup.sh" -ds

set_manager_log DM TRACE &&
    log -deb "dm/dm_setup.sh - Manager log for DM set to TRACE - Success" ||
    raise "FAIL: set_manager_log DM TRACE - Could not set manager log severity" -l "dm/dm_setup.sh" -ds
...
do
    wait_ovsdb_entry Wifi_Radio_State -w if_name "$if_name" -is if_name "$if_name" &&
        ...
        raise "FAIL: Wifi_Radio_State::if_name for '$if_name' does not exist" -l "dm/dm_setup.sh" -ds
done
