...
device_init &&
    ...
    raise "FAIL: device_init - Could not initialize device" -l "dm/othr_setup.sh" -ds

start_openswitch &&
    log -deb "dm/othr_setup.sh - OpenvSwitch started - Success" ||
    raise "FAIL: start_openswitch - Could not start OpenvSwitch" -l "dm/othr_setup.sh" -ds

restart_managers
log -deb "dm/othr_setup.sh: Executed restart_managers, exit code: $?"
...
do
    wait_ovsdb_entry Wifi_Radio_State -w if_name "$if_name" -is if_name "$if_name" &&
        log -deb "dm/othr_setup.sh - Wifi_Radio_State::if_name '$if_name' present - Success" ||
        raise "FAIL: Wifi_Radio_State::if_name for '$if_name' does not exist" -l "dm/othr_setup.sh" -ds
done
