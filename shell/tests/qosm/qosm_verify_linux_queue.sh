...
#
# Test basic Linux qdisc QoS config on an interface via Interface_QoS/Linux_Queue
#
...
${0} [-h] arguments
Description:
    - This script configures and validates basic Linux qdisc QoS config  on an OpenSync node by configuring
      IP_Interface/Interface_Qos/Linux_Queue hierarchy.
Arguments:
...
trap '
    ...
    echo -e "\n\n"
    fut_info_dump_line
    print_tables IP_Interface Interface_QoS Linux_Queue
    fut_info_dump_line
    reset_config
    exit $fut_ec
' EXIT INT TERM

log_title "${LOG_TAG}: Configuring and validating Linux qdisc QoS configuration via Interface_QoS/Linux_Queue"
...
[
        ...
        },
        {
            "op" : "insert",
            "table" : "Linux_Queue",
            "uuid-name": "lnx_queue_up_htb_class_voice",
            "row" : {
                "parent_id": "1:",
                "id": "1:1",
                "type": "class",
                "name": "htb",
                "params": "rate 237Mbit ceil 950Mbit"
            }
        },
        {
            "op" : "insert",
            "table" : "Linux_Queue",
            "uuid-name": "lnx_queue_up_htb_class_video",
            "row" : {
                "parent_id": "1:",
                "id": "1:2",
                "type": "class",
                "name": "htb",
                "params": "rate 713Mbit ceil 950Mbit"
            }
        },
        {
            "op" : "insert",
            "table" : "Linux_Queue",
            "uuid-name": "lnx_queue_up_pfifo_voice",
            "row" : {
                "parent_id": "1:1",
                "id": "100:",
                "type": "qdisc",
        ...
        },
        {
            "op" : "insert",
            "table" : "Linux_Queue",
            "uuid-name": "lnx_queue_up_pfifo_video",
            "row" : {
                "parent_id": "1:2",
                "id": "200:",
                "type": "qdisc",
        ...
        },
        {
            ...
            "row" : {
                "lnx_queues": ["set", [["named-uuid", "lnx_queue_up_htb"], ["named-uuid", "lnx_queue_up_htb_class_voice"], ["named-uuid", "lnx_queue_up_htb_class_video"], ["named-uuid", "lnx_queue_up_pfifo_voice"], ["named-uuid", "lnx_queue_up_pfifo_video"]]]
            }
        },
        {
            "op" : "insert",
...
log "${LOG_TAG}: Transacting IP_Interface/Interface_QoS/Linux_Queue ..."
...
wait_ovsdb_entry Interface_QoS \
    ...
    -is status "success" -t 5 &&
        log "${LOG_TAG}: Linux qdisc QoS configured - Success" ||
        raise "Failed configuring Linux qdisc QoS" -l "${LOG_TAG}" -fc

# Check if modification works:
update_ovsdb_entry Linux_Queue -w "id" "1:2" -u "params" "rate 373Mbit ceil 950Mbit"
...
wait_ovsdb_entry Interface_QoS \
    ...
    -is status "success" -t 5 &&
        log "${LOG_TAG}: Linux qdisc QoS configured - Success" ||
        raise "Failed modifying Linux qdisc QoS" -l "${LOG_TAG}" -fc
