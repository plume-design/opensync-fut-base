...
def qm_setup(request: pytest.FixtureRequest):
...
def test_qm_telog_validate(qm_setup, parametrized_test_config, gw_handler):
    with step("Preparation of testcase parameters"):
        number_of_logs = parametrized_test_config.get("number_of_logs")
        log_tail_command = gw_handler._get_log_tail_command()
        test_args = get_command_arguments(number_of_logs, log_tail_command, gw_handler.TEST_SCRIPT_TIMEOUT)

    with step("Test case"):
        assert gw_handler.execute_with_logging("tests/qm/qm_telog_validate", test_args)[0] == 0
